...
def test_subtotal_199_99_cobra_envio(client, producto_precio):
    ...
    res = client.post(
        "/shipping/quote",
        json={"items": [{"id": slug, "qty": 1}], "zip": "10001", "method": "standard"},
    )
    assert res.status_code == 200
    data = res.json()
    assert data["subtotal"] == 199.99
    assert data["shipping"] == 6.95
    assert data["total_estimate"] == 206.94
...
def test_metodo_express_bajo_el_umbral(client, producto_precio):
    slug = producto_precio("50.00")
    res = client.post(
        "/shipping/quote",
        json={"items": [{"id": slug, "qty": 2}], "zip": "10001", "method": "express"},
    )
    data = res.json()
    assert data["subtotal"] == 100
    assert data["shipping"] == 14.95
...
def test_zip_invalido_da_422(client, products):
    res = client.post(
        "/shipping/quote",
        json={"items": [{"id": "khamrah", "qty": 1}], "zip": "ABCDE", "method": "standard"},
    )
...
def test_qty_cero_da_422(client, products):
    res = client.post(
        "/shipping/quote",
        json={"items": [{"id": "khamrah", "qty": 0}], "zip": "10001", "method": "standard"},
    )
    assert res.status_code == 422
...
def test_producto_desconocido_da_404(client, products):
    res = client.post(
        "/shipping/quote",
        json={"items": [{"id": "fantasma", "qty": 1}], "zip": "10001", "method": "standard"},
    )
